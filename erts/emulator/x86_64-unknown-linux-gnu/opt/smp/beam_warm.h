...
{
  Eterm tmp_packed1 = I[2];
  if (!erts_bs_put_utf16(ERL_BITS_ARGS_2(Sb((tmp_packed1>>BEAM_LOOSE_SHIFT)), tb(tmp_packed1&BEAM_LOOSE_MASK)))) {
    c_p->freason = BADARG;
...
OpCase(bs_set_position_xx):
{
  ...
  mb = ms_matchbuffer(context);
  mb->offset = unsigned_val(xb((tmp_packed1>>BEAM_LOOSE_SHIFT)));
  I += 2;
  ASSERT(VALID_INSTR(next_pf));
  GotoPF(next_pf);
}
...
OpCase(bs_set_position_xy):
{
  ...
  mb = ms_matchbuffer(context);
  mb->offset = unsigned_val(yb((tmp_packed1>>BEAM_LOOSE_SHIFT)));
  I += 2;
  ASSERT(VALID_INSTR(next_pf));
  GotoPF(next_pf);
}
...
OpCase(bs_set_position_yx):
{
  ...
  context = yb(tmp_packed1&BEAM_LOOSE_MASK);
  ASSERT(header_is_bin_matchstate(*boxed_val(context)));
...
OpCase(bs_test_tail_imm2_fyW):
{
  Eterm tmp_packed1 = I[1];
  ErlBinMatchBuffer *_mb;
  _mb = ms_matchbuffer(yb((tmp_packed1>>BEAM_WIDE_SHIFT)));
  if (_mb->size - _mb->offset != I[2]) {
    ASSERT(VALID_INSTR(*(I + (fb(tmp_packed1&BEAM_WIDE_MASK)) + 0)));
    I += fb(tmp_packed1&BEAM_WIDE_MASK) + 0;;
    Goto(*I);;
...
OpCase(bs_test_unit_fyt):
{
  Eterm tmp_packed1 = I[2];
  ErlBinMatchBuffer *_mb;
  _mb = ms_matchbuffer(yb(tmp_packed1&BEAM_LOOSE_MASK));
  if ((_mb->size - _mb->offset) % tb((tmp_packed1>>BEAM_LOOSE_SHIFT))) {
    ASSERT(VALID_INSTR(*(I + (I[1]) + 0)));
    I += I[1] + 0;;
    Goto(*I);;
...
OpCase(bs_test_zero_tail2_fy):
{
  Eterm tmp_packed1 = I[1];
  ErlBinMatchBuffer *_mb;
  _mb = (ErlBinMatchBuffer*) ms_matchbuffer(yb((tmp_packed1>>BEAM_WIDE_SHIFT)));
  if (_mb->size != _mb->offset) {
    ASSERT(VALID_INSTR(*(I + (fb(tmp_packed1&BEAM_WIDE_MASK)) + 0)));
    I += fb(tmp_packed1&BEAM_WIDE_MASK) + 0;;
    Goto(*I);;
...

{
  ...
  i_bs_get_binary_imm2__execute:
  {
    ...
    Eterm _result;
    do {
      Uint need = heap_bin_size(ERL_ONHEAP_BIN_LIMIT);
      if (ERTS_UNLIKELY(E - HTOP < need)) {
        do {
          //
          // Since a garbage collection is expensive anyway, we can afford
    ...
    _mb = ms_matchbuffer(context);
    LIGHT_SWAPOUT;
    _result = erts_bs_get_binary_2(c_p, I[4], tb((tmp_packed1>>BEAM_TIGHT_SHIFT)&BEAM_TIGHT_MASK), _mb);
    LIGHT_SWAPIN;
    HEAP_SPACE_VERIFIED(0);
    if (is_non_value(_result)) {
      ASSERT(VALID_INSTR(*(I + (I[2]) + 0)));
...

{
  ...
  OpCase(i_bs_get_integer_imm_yWtftx):
  {
    Eterm tmp_packed1 = I[1];
    Uint wordsneeded;
    Ms = yb(tmp_packed1&BEAM_LOOSE_MASK);
    Sz = I[2];
    wordsneeded = 1+WSIZE(NBYTES(Sz));
    do {
      Uint need = wordsneeded;
...
OpCase(i_bs_get_position_xx):
{
  ...
  mb = ms_matchbuffer(context);
  xb((tmp_packed1>>BEAM_LOOSE_SHIFT)) = make_small(mb->offset);
  I += 2;
  ASSERT(VALID_INSTR(next_pf));
  GotoPF(next_pf);
}
...
OpCase(i_bs_get_position_xy):
{
  ...
  mb = ms_matchbuffer(context);
  yb((tmp_packed1>>BEAM_LOOSE_SHIFT)) = make_small(mb->offset);
  I += 2;
  ASSERT(VALID_INSTR(next_pf));
  GotoPF(next_pf);
}
...
OpCase(i_bs_match_string_yfWW):
{
  ...
  mb = ms_matchbuffer(yb(tmp_packed1&BEAM_WIDE_MASK));
  if (mb->size - mb->offset < bits) {
    ASSERT(VALID_INSTR(*(I + (fb((tmp_packed1>>BEAM_WIDE_SHIFT))) + 0)));
    I += fb((tmp_packed1>>BEAM_WIDE_SHIFT)) + 0;;
    Goto(*I);;
...
{
  Eterm tmp_packed1 = I[1];
  if (!erts_bs_put_utf8(ERL_BITS_ARGS_1(Sb((tmp_packed1>>BEAM_WIDE_SHIFT))))) {
    c_p->freason = BADARG;
...

{
  ...
  OpCase(i_bs_skip_bits2_xxft):
  {
    Eterm tmp_packed1 = I[1];
    context = xb(tmp_packed1&BEAM_LOOSE_MASK);
    bits = xb((tmp_packed1>>BEAM_LOOSE_SHIFT));
  }
  ...
  OpCase(i_bs_skip_bits2_xyft):
  {
    Eterm tmp_packed1 = I[1];
    context = xb(tmp_packed1&BEAM_LOOSE_MASK);
    bits = yb((tmp_packed1>>BEAM_LOOSE_SHIFT));
  }
  goto i_bs_skip_bits2__execute;
...
OpCase(i_bs_utf16_size_Sx):
{
  Eterm tmp_packed1 = I[1];
  BeamInstr next_pf = BeamCodeAddr(I[2]);
  Eterm arg = Sb(tmp_packed1&BEAM_LOOSE_MASK);
...
{
  Eterm tmp_packed1 = I[1];
  Eterm val = Sb((tmp_packed1>>BEAM_WIDE_SHIFT));
...
{
  Eterm tmp_packed1 = I[1];
  if (!erts_new_bs_put_binary(ERL_BITS_ARGS_2((Sb((tmp_packed1>>BEAM_WIDE_SHIFT))), (I[2])))) {
    c_p->freason = BADARG;
...
{
  Eterm tmp_packed1 = I[2];
  Eterm sz = Sb(tmp_packed1&BEAM_TIGHT_MASK);
  Sint _size;
  if (is_small(sz)) {
    Uint uint_size;
    ...
    uint_size = (Uint) signed_size;
    _size = uint_size * ((tb((tmp_packed1>>BEAM_TIGHT_SHIFT)&BEAM_TIGHT_MASK)) >> 3);
  } else {
    /*
    * On a 64-bit architecture, the size of any binary
    * that would fit in the memory fits in a small.
  ...
  if (!erts_new_bs_put_binary(ERL_BITS_ARGS_2((Sb((tmp_packed1>>(2*BEAM_TIGHT_SHIFT)))), _size))) {
    c_p->freason = BADARG;
...
OpCase(i_new_bs_put_float_jSts):
{
  ...
  GetSource(I[3], targ1);
  {
    Eterm sz = Sb(tmp_packed1&BEAM_LOOSE_MASK);
    Eterm flags = tb((tmp_packed1>>BEAM_LOOSE_SHIFT));
    Sint _size;
    if (is_small(sz)) {
      Uint uint_size;
      Sint signed_size = signed_val(sz);
...

{
  ...
  {
    Eterm tmp_packed1 = I[2];
    if (!erts_new_bs_put_integer(ERL_BITS_ARGS_3(src, (I[3]), (tb((tmp_packed1>>BEAM_WIDE_SHIFT)))))) {
      c_p->freason = BADARG;
