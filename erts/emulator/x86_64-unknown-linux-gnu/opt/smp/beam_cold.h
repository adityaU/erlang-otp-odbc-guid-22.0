...
OpCase(cold_is_function2_fxx):
{
  ...
  if (erl_is_function(c_p, xb(tmp_packed1&BEAM_LOOSE_MASK), xb((tmp_packed1>>BEAM_LOOSE_SHIFT))) != am_true ) {
    ASSERT(VALID_INSTR(*(I + (I[1]) + 0)));
    I += I[1] + 0;;
    Goto(*I);;
  }
...
OpCase(i_bs_restore2_xt):
{
  ...
  ASSERT(HEADER_NUM_SLOTS(_ms->thing_word) > tb((tmp_packed1>>BEAM_LOOSE_SHIFT)));
